...
///
/// A different type from [`Rp2040I2c`], not just a different way of using it:
...
/// (`embassy-rp`'s `time-driver` feature backs it, on the RP2040 timer) rather
/// than the blocking board's cycle counting [`CycleDelay`].
///
/// Everything [`uferris_init`] documents about [`embassy_rp::init`], the second
/// stage bootloader and the absent console holds here unchanged. Waiting on the
