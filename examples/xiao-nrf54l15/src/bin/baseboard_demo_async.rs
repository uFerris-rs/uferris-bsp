...
/// The shared demos take a [`core::fmt::Write`] sink, and `rtt-target` hands
/// the channel it opens to the printing macros rather than back to the caller,
/// so this forwards to `rprint!`. That keeps the demo output, and the panic
/// message `panic-rtt-target` prints, on one and the same RTT channel.
struct RttWriter;
...
async fn main(_spawner: Spawner) -> ! {
    ...
    // Its button 5 phase is the one to watch: it suspends on the pin rather
    // than spinning on it.
    uferris_demos::asynch::baseboard_demo(&mut uferris, &mut delay, &mut RttWriter).await
}
