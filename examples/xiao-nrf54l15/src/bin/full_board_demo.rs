...
fn main() -> ! {
    ...
    loop {
        ...
        rprintln!("Battery Voltage: {:?} mV", voltage);
        rprintln!("Battery Current: {:?} uA", current);
        rprintln!("Battery Power: {:?} uW", power);
        ...
        match sd_card {
            Ok(vol_mgr) => {
                ...
                rprintln!("SD Card Size: {} Bytes", vol_mgr);

                uferris.write_to_file_in_root("TEST.TXT", "hello".as_bytes());
                rprintln!("Wrote 'hello' to 'TEXT.TXT' file in root directory.");

                rprintln!("Reading 'TEST.TXT' file from root directory:");
                let result = uferris.read_file_chunked("TEST.TXT", |chunk| {
                    rprintln!("Chunk: {:?}", core::str::from_utf8(chunk).unwrap_or("??"));
                });

                if result.is_err() {
                    rprintln!("Read error occurred!");
                }
            }
            Err(e) => {
                rprintln!("Failed to initialize SD Card: {:?}", e);
            }
        }
