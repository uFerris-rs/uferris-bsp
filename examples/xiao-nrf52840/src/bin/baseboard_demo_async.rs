...
async fn main(_spawner: Spawner) -> ! {
    ...
    // apply the chip errata workarounds and hand back the peripheral
    // singletons. What is different here is what the crate was built with — the
    // `async` feature turns on the RTC1 time driver behind `embassy-time`, and
    // the GPIOTE interrupt `init` also brings up, which is what lets the board
    // wait on a pin.
    ...
    // by the RTC1 driver. The blocking examples count CPU cycles instead.
